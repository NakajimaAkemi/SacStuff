...
class MyForm(Form):
    id = IntegerField('id', [validators.NumberRange(min=0,max=255)])
    nome = StringField('nome', [validators.length(max=10)])
    cognome = StringField('cognome', [validators.length(max=10)])
